...
function getAssetNameFromModule(module: NormalModule): string | null {
    ...
    const buildInfo = module.buildInfo as { assets?: Record<string, unknown> } | undefined;
...
function hasDeclaration(
    declarations: (CssDeclaration | { type: string; property?: string; value?: string })[],
    property: string,
    value: string
): boolean {
...
function isConcatSource(asset: sources.Source): asset is ConcatSourceLike {
    ...
    return concatAsset.children !== undefined && Array.isArray(concatAsset.children);
}
...
class ImageSpritePlugin {
        ...
        if (isConcatSource(asset)) {
            ...
            asset.children!.forEach((childAsset: sources.Source, i: number) => {
                (this._cssAstMap[assetName] as CssStylesheet[])[i] = css.parse(
                    childAsset.source().toString(), { source: `source${i}.css` });
            });
        } else {
    ...
    private createSprite(
        ...
        callback: (sprite: Buffer | null, coordinates: Record<string, SpritesmithCoordinates> | null) => void
    ): void {
        ...
        }, (err: Error | null, result: { image: Buffer; coordinates: Record<string, SpritesmithCoordinates> } | undefined) => {
            if (err) {
                logger.error(err);
            } else if (result) {
    ...
    private getChangedChunks(compilation: Compilation): Array<{ name?: string | null; hash?: string; files: Set<string> }> {
    ...
    private getCoordinates(module: NormalModule): SpritesmithCoordinates | undefined {
    ...
    private getImageAssetCandidates(compilation: Compilation): Record<string, sources.Source> {
    ...
    private getImageModules(compilation: Compilation): NormalModule[] {
        ...
        return modules.filter((module): module is NormalModule => {
            ...
            const request = normalModule.rawRequest || normalModule.userRequest || normalModule.resource || '';
    ...
    private parseCssBlock(block: string): ParsedCssBlock {
        const propertyMap: Record<string, { declaration: CssDeclaration; index: number }> = {};
        ...
        if (Array.isArray(obj)) {
            ...
            obj.forEach((ast, i) => {
                ...
                concatAsset.children![i] = new SourceMapSource(
                    cssSource, '', child._sourceMap as string);
            });
        } else {
            ...
            compilation.updateAsset(assetName, new SourceMapSource(
                cssSource, assetName, sourceMap as unknown as string));
        }
    }
    ...
    private updateCssBackgroundImage(decl: CssDeclaration, index: number): void {
        ...
        declarations.some((declaration, i) => {
            if (declaration.type === 'declaration' && (declaration as CssDeclaration).property === 'background-position') {
                indexPos = i;
    ...
    private updateCssBackgroundShorthand(decl: CssDeclaration, index: number): void {
        const decValue = decl.value;
        const { url, local } = this.getImagePathFromStyle(decValue);
        const coord = this._coordinates?.[local];
        if (!coord) {
    ...
    private updateSprite(sprite: Buffer, coordinates: Record<string, SpritesmithCoordinates>): void {
